...
public:
  NMPCController() : Node("NMPC_controller") {
    ...
    // Subscriptions // sensordata qos na pudhusa erukara best states dha eduthukum ilana normal ah 10 buffer size vechekalam
    sub_failure_ = create_subscription<FailureCheck>(
        "/agents/failure_check", 10,
        std::bind(&NMPCController::failureCheckCb, this, std::placeholders::_1));
    
    sub_uav_pose_3 = create_subscription<RigidBodyPose>(
        "/x500_3/pose", rclcpp::SensorDataQoS(),
        std::bind(&NMPCController::uavPoseCb3, this, std::placeholders::_1));

    sub_uav_pose_2 = create_subscription<RigidBodyPose>(
        "/x500_2/pose", rclcpp::SensorDataQoS(),
        std::bind(&NMPCController::uavPoseCb2, this, std::placeholders::_1));

    sub_payload_ = create_subscription<RigidBodyPose>(
        "/payload/pose", 10,
        std::bind(&NMPCController::payloadCb, this, std::placeholders::_1));

        // Edhu theva ila direct ah 30 states use panum pothu..
    ...
    control_timer_ = create_wall_timer(
      ...
      std::bind(&NMPCController::controlLoop, this));

    RCLCPP_INFO(get_logger(),
                "NMPC controller for uav2 & uav3 BOOM BOOM!! - dt = %.3f s, horizon = %u",
                dt_, N_);
  }
...
private:
  ...
  // rclcpp::Subscription<JointForceTorque>::SharedPtr sub_uav2_tension_; try multi thread
  // declaration as the member keep it live as long as the class is live
  ...
  std::chrono::high_resolution_clock::time_point last_cycle_time_; // To measure time between cycles
  ...
  void declare_parameters() {
    ...
    // Tether parameters edhu mathanum !!! gazebo la epadi eruko apadi mathanum !!!
    declare_parameter("L", 1.8);         // Tether length
    declare_parameter("k", 80.0);        // Spring constant
    declare_parameter("b", 2.0);         // Damping constant 

    declare_parameter("dt", 0.02);       // MATLAB: dt = 0.02
    declare_parameter("N",  20);         // MATLAB: N = 30

    // Formation parameter - MATLAB: dis_between_uav = 1.2
    declare_parameter("dis_between_uav", 1.2); 

    declare_parameter("T_MIN", 10.0);
    declare_parameter("T_MAX", 33.0);
    declare_parameter("TAU_ROLL_MIN",  -0.2); //andrea kitta kekkanum !! change this too
    declare_parameter("TAU_ROLL_MAX",   0.2);
    declare_parameter("TAU_PITCH_MIN", -0.2);
    declare_parameter("TAU_PITCH_MAX",  0.2);
  ...
  void uavPoseCb2(const RigidBodyPose::SharedPtr msg) {
    ...
    phi_dot_uav2   = msg->ang_velocity[0]; // p i changed the gazebo setting so i get directly body frame 
  ...
  void failureCheckCb(const FailureCheck::SharedPtr msg) {
    if (!active_ && (msg->uav_1_failed || msg->uav_2_failed || msg->uav_3_failed)) {
      ...
      RCLCPP_WARN(get_logger(),
                  "=====Failure detected - Activating MPC======");
    }
  }
  ...
  void controlLoop() {
    ...
    // Pack current state vector [30x1] matching
    casadi::DM P = casadi::DM::zeros(30, 1);
    ...
    // Increment debug counter and record timestamp
    debug_counter_++;
    ...
    // Calculate and print tether distances for debugging
    // double r1p_x = pN_payload - pN_uav2;
    // double r1p_y = pE_payload - pE_uav2; 
    // double r1p_z = pD_payload - pD_uav2;
    // double d1 = sqrt(r1p_x*r1p_x + r1p_y*r1p_y + r1p_z*r1p_z);
    
    // double r2p_x = pN_payload - pN_uav3;
    // double r2p_y = pE_payload - pE_uav3;
    // double r2p_z = pD_payload - pD_uav3;
    // double d2 = sqrt(r2p_x*r2p_x + r2p_y*r2p_y + r2p_z*r2p_z);
    
    // double uav_distance = sqrt((pN_uav2-pN_uav3)*(pN_uav2-pN_uav3) + 
    //                           (pE_uav2-pE_uav3)*(pE_uav2-pE_uav3) + 
    //                           (pD_uav2-pD_uav3)*(pD_uav2-pD_uav3));
    
    // std::cout << "Tether1 dist=" << d1 << " (nominal=" << L_ << ") Tether2 dist=" << d2 
    //           << " UAV-UAV dist=" << uav_distance << std::endl;
    ...
    if (w0.is_empty()) {
      ...
      // RCLCPP_INFO(get_logger(), "Initialized warm start with current states (hover_thrust=%.2f)", hover_thrust);
    }
    ...
    try {
      ...
      // UAV1 control (UAV2 in ROS). !!!! changed this !!!!
      ...
      // DEBUG: Print MPC optimal control outputs
      ...
      // std::cout << "UAV2 Published: [" << tau_roll_uav2 << ", " << tau_pitch_uav2 
      //           << ", " << -thrust_uav2 << ", " << tau_yaw_uav2 << "]" << std::endl;
      // std::cout << "UAV3 Published: [" << tau_roll_uav3 << ", " << tau_pitch_uav3 
      //           << ", " << -thrust_uav3 << ", " << tau_yaw_uav3 << "]" << std::endl;
      ...
      cmd_uav3.timestamp = 1; //debug my control and andrea andrea is always 0
      cmd_uav3.setpoint[0] = static_cast<float>(tau_roll_uav3);
      cmd_uav3.setpoint[1] = static_cast<float>(tau_pitch_uav3);
      cmd_uav3.setpoint[2] = static_cast<float>(-thrust_uav3); //px4 need -thrust
      ...
      std::cout << "=== END MPC DEBUG CYCLE ===" << std::endl << std::endl;
  ...
  // --------------------------------------------------------------------------
  // Build MPC solver (MATLAB matching dynamics with tether forces)
  void build_mpc_solver() {
    ...
    SX P = SX::sym("P", 30);             // Parameters (current state)
    ...
    g.push_back(X(Slice(), 0) - P); //xk(:, 0) matlab maari dha indexing X(:,0) = P
    ...
    for (unsigned int k = 0; k < N_; ++k) {
      ...
      // Position vectors
      SX p1_vec[3] = {N1, E1, D1};
      SX p2_vec[3] = {N2, E2, D2};
      SX pp_vec[3] = {Np, Ep, Dp};
      SX v1_vec[3] = {vN1, vE1, vD1};
      SX v2_vec[3] = {vN2, vE2, vD2};
      SX vp_vec[3] = {vNp, vEp, vDp};
      SX eps = 1e-8;
      
      // Tether 1 (UAV1 to payload) - MATLAB matching
      SX r1p[3] = {pp_vec[0] - p1_vec[0], pp_vec[1] - p1_vec[1], pp_vec[2] - p1_vec[2]};
      SX r1p_norm_sq = r1p[0]*r1p[0] + r1p[1]*r1p[1] + r1p[2]*r1p[2];
      SX d1 = sqrt(r1p_norm_sq + 1e-8);  // Add small constant for numerical stability
      
      SX d1_safe = fmax(d1, 1e-6);  // MATLAB: fmax(d1, 1e-6)
      SX u1t[3] = {r1p[0]/d1_safe, r1p[1]/d1_safe, r1p[2]/d1_safe};
      
      // Spring force: k*(d1 - L)*u1t
      SX F1_spring[3] = {k_*(d1 - L_)*u1t[0], k_*(d1 - L_)*u1t[1], k_*(d1 - L_)*u1t[2]};
      
      // Damping force: b * dot(vel_rel1, u1t) * u1t
      SX vel_rel1[3] = {vp_vec[0] - v1_vec[0], vp_vec[1] - v1_vec[1], vp_vec[2] - v1_vec[2]};
      SX F1_damping[3] = {b_*vel_rel1[0], b_*vel_rel1[1], b_*vel_rel1[2]};

      // Total force F1 = F1_spring + F1_damping
      SX F1[3] = {F1_spring[0] + F1_damping[0], F1_spring[1] + F1_damping[1], F1_spring[2] + F1_damping[2]};

      // Tether 2 (UAV2 to payload) - MATLAB matching
      SX r2p[3] = {pp_vec[0] - p2_vec[0], pp_vec[1] - p2_vec[1], pp_vec[2] - p2_vec[2]};
      SX r2p_norm_sq = r2p[0]*r2p[0] + r2p[1]*r2p[1] + r2p[2]*r2p[2];
      SX d2 = sqrt(r2p_norm_sq + 1e-8);  // Add small constant for numerical stability
      
      SX d2_safe = fmax(d2, 1e-6);  // MATLAB: fmax(d2, 1e-6)
      SX u2t[3] = {r2p[0]/d2_safe, r2p[1]/d2_safe, r2p[2]/d2_safe};
      
      // Spring force: k*(d2 - L)*u2t
      SX F2_spring[3] = {k_*(d2 - L_)*u2t[0], k_*(d2 - L_)*u2t[1], k_*(d2 - L_)*u2t[2]};
      
      // Damping force: b * dot(vel_rel2, u2t) * u2t
      SX vel_rel2[3] = {vp_vec[0] - v2_vec[0], vp_vec[1] - v2_vec[1], vp_vec[2] - v2_vec[2]};
      SX F2_damping[3] = {b_*vel_rel2[0], b_*vel_rel2[1], b_*vel_rel2[2]};

      // Total force F2 = F2_spring + F2_damping
      SX F2[3] = {F2_spring[0] + F2_damping[0], F2_spring[1] + F2_damping[1], F2_spring[2] + F2_damping[2]};
      ...
      // 1. Payload oscillation damping - MATLAB: J = J + 50.0 * (vNp^2 + vEp^2 + vDp^2);
      SX payload_vel_cost = w_payload_vel_ * (vNp_cost*vNp_cost + vEp_cost*vEp_cost + vDp_cost*vDp_cost);
      
      // 2. Payload centering - MATLAB: payload_center_N = (N1 + N2) / 2; J = J + 150.0 * ((Np - payload_center_N)^2 + (Ep - payload_center_E)^2);
      ...
      // 3. Payload acceleration penalty - MATLAB: if k > 1 ... J = J + 5.0 * (aNp^2 + aEp^2 + aDp^2);
      ...
      // 4. UAV velocity coordination - MATLAB: J = J + 2 * ((vN1 - vN2)^2 + (vE1 - vE2)^2 + (vD1 - vD2)^2);
      ...
      // 5. UAV formation constraints - MATLAB: J = J + 1 * ((N1 - N2)^2 - dis_between_uav^2)^2; J = J + 1 * ((E1 - E2)^2 - dis_between_uav^2)^2;
      ...
      // 6. Control effort - based on Andrea asked !!!! not adding the control no difference 
    ...
    // MATLAB: payload_vel_final = sqrt(vNpT^2 + vEpT^2 + vDpT^2); J = J + 100.0 * payload_vel_final^2;
    SX vNpT = xT(25), vEpT = xT(27), vDpT = xT(29);
    SX payload_vel_final_sq = vNpT*vNpT + vEpT*vEpT + vDpT*vDpT;  // Use squared directly to avoid sqrt
    cost += w_terminal_vel_ * payload_vel_final_sq;

    // MATLAB: payload_center_NT = (N1T + N2T) / 2; J = J + 30.0 * ((NpT - payload_center_NT)^2 + (EpT - payload_center_ET)^2);
    ...
    // MATLAB: J = J + 35.0 * (vN1T^2 + vE1T^2 + vD1T^2 + vN2T^2 + vE2T^2 + vD2T^2);
    ...
    SX w = SX::vertcat({SX::reshape(X, 30*(N_+1), 1),  
                        SX::reshape(U,  8*N_,      1)});
    SX g_concat = SX::vertcat(g); //x(:0) - P , x(:,1) - f(x(:,0), u(:,0)) , ...
    ...
    Dict opts;
    opts["ipopt.print_level"] = 0;               // Disable printing for speed
    opts["print_time"] = false;                  
    opts["ipopt.sb"] = "yes";                    
    opts["ipopt.max_iter"] = 50;                 // Reduce iterations for real-time
    opts["ipopt.tol"] = 1e-2;                    // Relaxed tolerance for speed
    ...
    opts["ipopt.linear_solver"] = "mumps";       // Parallel linear solver
    opts["ipopt.mumps_mem_percent"] = 1000;      // More memory for speed
    ...
    // states measured at 1st should be equal to the parameters (current state)
    // so we set the bounds accordingly
