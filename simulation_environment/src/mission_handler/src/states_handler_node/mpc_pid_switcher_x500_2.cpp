...
class MPCPIDSwitcherX500_2 : public rclcpp::Node {
public:
  MPCPIDSwitcherX500_2()
  : Node("mpc_pid_switcher_x500_2")
  {
    // Subscriptions to MPC and PID topics
    sub_mpc_ = create_subscription<InputSetpoint>(
      "/x500_2/input_setpoint_mpc", 10,
      std::bind(&MPCPIDSwitcherX500_2::mpcCallback, this, std::placeholders::_1));

    sub_pid_ = create_subscription<InputSetpoint>(
      "/x500_2/input_setpoint_pid", 10,
      std::bind(&MPCPIDSwitcherX500_2::pidCallback, this, std::placeholders::_1));

    // Publisher to the final input_setpoint topic
    pub_setpoint_ = create_publisher<InputSetpoint>("/x500_2/input_setpoint", 10);

    RCLCPP_INFO(this->get_logger(), "MPC/PID Switcher Node started for x500_2 (Event-driven)");
  }
...
private:
  void mpcCallback(const InputSetpoint::SharedPtr msg) {
    ...
    if (!mpc_active_) {
      mpc_active_ = true;
      RCLCPP_INFO(this->get_logger(), "Switching to MPC control for x500_2");
    }
  ...
  void pidCallback(const InputSetpoint::SharedPtr msg) {
    ...
    if (!mpc_active_ || !mpc_fresh) {
      if (mpc_active_ && !mpc_fresh) {
        RCLCPP_WARN(this->get_logger(), "MPC data stale, using PID for x500_2");
      }
      pub_setpoint_->publish(*msg);
    }
  }
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<MPCPIDSwitcherX500_2>());
  rclcpp::shutdown();
  return 0;
}