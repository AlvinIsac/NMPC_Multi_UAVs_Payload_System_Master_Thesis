...
dt = 0.05;       
N  = 10;         
MAX_STEPS = 250;  % to check failure mode
nx = 30; nu = 8;                   
...
%% CasADi Dynamics Function with RK4 Integration
function xdot = continuous_dynamics_2uav_casadi(x, u, Ixx, Iyy, Izz, m, mP, L, k, b, g)
    ...
    cth1 = cos(th1); cth1 = fmax(abs(cth1), 1e-6) * sign(cth1);
    eul1_dot1 = p1 + sin(phi1)*tan(th1)*q1 + cos(phi1)*tan(th1)*r1;
    eul1_dot2 = cos(phi1)*q1 - sin(phi1)*r1;
    eul1_dot3 = sin(phi1)/cth1*q1 + cos(phi1)/cth1*r1;
    
    cth2 = cos(th2); cth2 = fmax(abs(cth2), 1e-6) * sign(cth2);
    eul2_dot1 = p2 + sin(phi2)*tan(th2)*q2 + cos(phi2)*tan(th2)*r2;
    eul2_dot2 = cos(phi2)*q2 - sin(phi2)*r2;
    eul2_dot3 = sin(phi2)/cth2*q2 + cos(phi2)/cth2*r2;
    ...
    % Pack state derivatives (continuous-time dynamics)
    x1_dot = [vN1; aN1; vE1; aE1; vD1; aD1; eul1_dot1; p1_dot; eul1_dot2; q1_dot; eul1_dot3; r1_dot];
    x2_dot = [vN2; aN2; vE2; aE2; vD2; aD2; eul2_dot1; p2_dot; eul2_dot2; q2_dot; eul2_dot3; r2_dot];
    xp_dot = [vNp; aNp; vEp; aEp; vDp; aDp];
    
    xdot = [x1_dot; x2_dot; xp_dot];
end

function xnext = step_dynamics_2uav_casadi_rk4(x, u, Ixx, Iyy, Izz, dt, m, mP, L, k, b, g)
    % RK4 Integration for better numerical accuracy
    
    % k1 = f(x, u)
    k1 = continuous_dynamics_2uav_casadi(x, u, Ixx, Iyy, Izz, m, mP, L, k, b, g);
    
    % k2 = f(x + 0.5*dt*k1, u)
    k2 = continuous_dynamics_2uav_casadi(x + 0.5*dt*k1, u, Ixx, Iyy, Izz, m, mP, L, k, b, g);
    
    % k3 = f(x + 0.5*dt*k2, u)
    k3 = continuous_dynamics_2uav_casadi(x + 0.5*dt*k2, u, Ixx, Iyy, Izz, m, mP, L, k, b, g);
    
    % k4 = f(x + dt*k3, u)
    k4 = continuous_dynamics_2uav_casadi(x + dt*k3, u, Ixx, Iyy, Izz, m, mP, L, k, b, g);
    
    % RK4 formula: x_next = x + (dt/6)*(k1 + 2*k2 + 2*k3 + k4)
    xnext = x + (dt/6)*(k1 + 2*k2 + 2*k3 + k4);
end

% Create CasADi dynamics function with RK4 integration
x_sym = SX.sym('x', nx);
u_sym = SX.sym('u', nu);
f_casadi = Function('f', {x_sym, u_sym}, {step_dynamics_2uav_casadi_rk4(x_sym, u_sym, Ixx, Iyy, Izz, dt, m, mP, L, k, b, g)});
...
% Configure IPOPT solver with settings similar to fmincon
ipopt_opts = struct(...
    'print_level', 0, ...
    'max_iter', 100, ...
    'tol', 1e-8, ...
...
while k <= MAX_STEPS
    ...
    u0 = Uopt(:,1);
    % u0 = zeros(nu, 1); % Uncomment to check failure mode
...
% Add legend (show UAV markers, payload box, tethers and both UAVs' attitude)
legend([uav1_body_x, uav2_body_x, payload_box_faces(1), tether1_line, tether2_line, ...
