...
%   from the workspace. Make sure to run uav_mpc_target.m first.
    ...
    % Animation loop - SMOOTH AND QUICK REAL-TIME
    % Create interpolated smooth trajectory for smoother animation
    interp_factor = 3;  % Interpolate 3x more points for smoothness
    t_smooth = linspace(t(1), t(end), length(t) * interp_factor);
    
    % Interpolate all trajectories for smooth animation
    N1_smooth = interp1(t, N1_traj, t_smooth, 'spline');
    E1_smooth = interp1(t, E1_traj, t_smooth, 'spline');
    D1_smooth = interp1(t, D1_traj, t_smooth, 'spline');
    N2_smooth = interp1(t, N2_traj, t_smooth, 'spline');
    E2_smooth = interp1(t, E2_traj, t_smooth, 'spline');
    D2_smooth = interp1(t, D2_traj, t_smooth, 'spline');
    Np_smooth = interp1(t, Np_traj, t_smooth, 'spline');
    Ep_smooth = interp1(t, Ep_traj, t_smooth, 'spline');
    Dp_smooth = interp1(t, Dp_traj, t_smooth, 'spline');
    
    % Interpolate attitude data
    phi1_smooth = interp1(t, phi1_traj, t_smooth, 'spline');
    theta1_smooth = interp1(t, theta1_traj, t_smooth, 'spline');
    psi1_smooth = interp1(t, psi1_traj, t_smooth, 'spline');
    phi2_smooth = interp1(t, phi2_traj, t_smooth, 'spline');
    theta2_smooth = interp1(t, theta2_traj, t_smooth, 'spline');
    psi2_smooth = interp1(t, psi2_traj, t_smooth, 'spline');
    
    % Smooth center trajectory
    center_N_smooth = (N1_smooth + N2_smooth) / 2;
    center_E_smooth = (E1_smooth + E2_smooth) / 2;
    center_D_smooth = (D1_smooth + D2_smooth) / 2;
    
    for i = 1:length(t_smooth)
        if ~ishandle(f_anim), break; end  % Stop if figure is closed
        
        % Update trajectory lines using smooth data
        set(traj1_line, 'XData', N1_smooth(1:i), 'YData', E1_smooth(1:i), 'ZData', D1_smooth(1:i));
        set(traj2_line, 'XData', N2_smooth(1:i), 'YData', E2_smooth(1:i), 'ZData', D2_smooth(1:i));
        set(trajp_line, 'XData', Np_smooth(1:i), 'YData', Ep_smooth(1:i), 'ZData', Dp_smooth(1:i));
        
        % Update center trajectory
        set(center_traj_line, 'XData', center_N_smooth(1:i), 'YData', center_E_smooth(1:i), 'ZData', center_D_smooth(1:i));
        
        % Update UAV1 drone body position and orientation
        current_pos1 = [N1_smooth(i), E1_smooth(i), D1_smooth(i)];
        current_att1 = [phi1_smooth(i), theta1_smooth(i), psi1_smooth(i)];
        ...
        current_pos2 = [N2_smooth(i), E2_smooth(i), D2_smooth(i)];
        current_att2 = [phi2_smooth(i), theta2_smooth(i), psi2_smooth(i)];
        ...
        current_payload_pos = [Np_smooth(i), Ep_smooth(i), Dp_smooth(i)];
        for j = 1:6
        ...
        % Update tether lines
        set(tether1_line, 'XData', [N1_smooth(i) Np_smooth(i)], 'YData', [E1_smooth(i) Ep_smooth(i)], 'ZData', [D1_smooth(i) Dp_smooth(i)]);
        set(tether2_line, 'XData', [N2_smooth(i) Np_smooth(i)], 'YData', [E2_smooth(i) Ep_smooth(i)], 'ZData', [D2_smooth(i) Dp_smooth(i)]);
        
        % Update orientation lines for UAV1
        x1_ned = R1 * (uav_size*[1;0;0]);
        y1_ned = R1 * (uav_size*[0;1;0]);
        z1_ned = R1 * (uav_size*[0;0;1]);
        set(roll1_line, 'XData', N1_smooth(i), 'YData', E1_smooth(i), 'ZData', D1_smooth(i), ...
                      'UData', x1_ned(1), 'VData', x1_ned(2), 'WData', x1_ned(3));
        set(pitch1_line, 'XData', N1_smooth(i), 'YData', E1_smooth(i), 'ZData', D1_smooth(i), ...
                       'UData', y1_ned(1), 'VData', y1_ned(2), 'WData', y1_ned(3));
        set(yaw1_line, 'XData', N1_smooth(i), 'YData', E1_smooth(i), 'ZData', D1_smooth(i), ...
        ...
        set(roll2_line, 'XData', N2_smooth(i), 'YData', E2_smooth(i), 'ZData', D2_smooth(i), ...
                      'UData', x2_ned(1), 'VData', x2_ned(2), 'WData', x2_ned(3));
        set(pitch2_line, 'XData', N2_smooth(i), 'YData', E2_smooth(i), 'ZData', D2_smooth(i), ...
                       'UData', y2_ned(1), 'VData', y2_ned(2), 'WData', y2_ned(3));
        set(yaw2_line, 'XData', N2_smooth(i), 'YData', E2_smooth(i), 'ZData', D2_smooth(i), ...
                     'UData', z2_ned(1), 'VData', z2_ned(2), 'WData', z2_ned(3));

        % Calculate metrics for display using smooth data
        tether_length1 = norm([N1_smooth(i) - Np_smooth(i); E1_smooth(i) - Ep_smooth(i); D1_smooth(i) - Dp_smooth(i)]);
        tether_length2 = norm([N2_smooth(i) - Np_smooth(i); E2_smooth(i) - Ep_smooth(i); D2_smooth(i) - Dp_smooth(i)]);
        
        % Enhanced title with metrics
        if has_target
            current_center_pos = [center_N_smooth(i), center_E_smooth(i), center_D_smooth(i)];
            distance_to_target = norm(current_center_pos - [Ndes, Edes, Ddes]);
            time_text = sprintf('Time: %.2f s | Target Dist: %.2f m | Tethers: %.2f, %.2f m', ...
                t_smooth(i), distance_to_target, tether_length1, tether_length2);
        else
            time_text = sprintf('Time: %.2f s | Tethers: %.2f, %.2f m', ...
                t_smooth(i), tether_length1, tether_length2);
        end
        title(['UAV-Payload Animation Replay (NED) - ', time_text]);
        
        % Smooth and quick real-time animation
        drawnow;
        pause(0.1);  % Faster and smoother - 5ms between frames
    end
