...
start_Np    = (pN1 + pN2) / 2;   
start_vNp   = 0.0;               
start_Ep    = 0.8;    % try the swing front or back 0.8 or -0.8
start_vEp   = 0.1;               
start_Dp    = pD1 + L ;          
start_vDp   = 0.0;               
...
for k = 1:N
    ...
    % STABILIZATION COST (when phase_param = 0) - ARRANGED BY DESCENDING WEIGHT
    stabilization_cost = 0;
    
    % Weight: 50.0 - PRIMARY GOAL: Kill payload oscillations VERY QUICKLY (highest priority)
    stabilization_cost = stabilization_cost + 50.0 * (vNp^2 + vEp^2 + vDp^2);
    
    % Weight: 15.0 - Add payload position penalty to center it between UAVs
    payload_center_N = (N1 + N2) / 2;
    payload_center_E = (E1 + E2) / 2;
    stabilization_cost = stabilization_cost + 15.0 * ((Np - payload_center_N)^2 + (Ep - payload_center_E)^2);
    
    % Weight: 5.0 - Payload acceleration penalty
    if k > 1
        xk_prev = X(:,k-1);
        ...
        aEp = (vEp - vEp_prev) / dt;
        aDp = (vDp - vDp_prev) / dt;
        stabilization_cost = stabilization_cost + 5.0 * (aNp^2 + aEp^2 + aDp^2);
    end
    
    % Weight: 2.0 - UAV velocity coordination
    stabilization_cost = stabilization_cost + 2 * ((vN1 - vN2)^2 + (vE1 - vE2)^2 + (vD1 - vD2)^2);
    
    % Weight: 1.0 - UAV formation constraints
    stabilization_cost = stabilization_cost + 1 * ((N1 - N2)^2 - dis_between_uav^2)^2;
    stabilization_cost = stabilization_cost + 1 * ((E1 - E2)^2 - dis_between_uav^2)^2;
    
    % Weight: w_u (0.03, 0.015, 0.015, 0.015) - Control effort
    stabilization_cost = stabilization_cost + u1'*w_u*u1 + u2'*w_u*u2;
    
    % TRACKING COST (when phase_param = 1) - ARRANGED BY DESCENDING WEIGHT
    tracking_cost = 0;
    max_individual_vel = 0.4;
    
    % Weight: 120.0 - 3. Maintain exact same altitude
    altitude_difference = D1 - D2;
    tracking_cost = tracking_cost + 120.0 * altitude_difference^2;
    
    % Weight: 80.0 - 7. Payload oscillation damping
    tracking_cost = tracking_cost + 80.0 * (vNp^2 + vEp^2 + vDp^2);
    
    % Weight: 50.0 - 2. Maintain exact UAV horizontal separation
    ...
    % Weight: 40.0 - 7b. Additional payload stability
    if k > 1
        xk_prev = X(:,k-1);
        vNp_prev = xk_prev(26); vEp_prev = xk_prev(28); vDp_prev = xk_prev(30);
    ...
    % Weight: 30.0 - 5. Perfect velocity coordination between UAVs
    tracking_cost = tracking_cost + 30.0 * ((vN1 - vN2)^2 + (vE1 - vE2)^2 + (vD1 - vD2)^2);
    
    % % Weight: 25.0 - 4. Keep individual UAV velocities small (penalty if exceeding max)
    ...
    %% Weight: 15.0 - 8. Keep payload naturally centered
    % payload_center_N = (N1 + N2) / 2;
    % payload_center_E = (E1 + E2) / 2;
    % tracking_cost = tracking_cost + 15.0 * ((Np - payload_center_N)^2 + (Ep - payload_center_E)^2);
    
    % % Weight: 12.0 (variable) - 1. Formation center tracking to target
    ...
    % % Weight: 10.0 - 6. Smooth formation movement toward target
    ...
    % % Weight: 8.0 - 4. Individual UAV velocity minimization (always applied)
    % tracking_cost = tracking_cost + 8.0 * (vel1_magnitude^2 + vel2_magnitude^2);
    
    % Weight: 8.0 - 12. Formation orientation consistency
    ...
    % Weight: 5.0 - 9. Attitude stability
    % tracking_cost = tracking_cost + 5.0 * (phi1^2 + theta1^2 + phi2^2 + theta2^2);
    
    % % Weight: 3.0 - 10. Angular velocity minimization
    % tracking_cost = tracking_cost + 3.0 * (p1^2 + q1^2 + r1^2 + p2^2 + q2^2 + r2^2);
    
    % % Weight: w_u (0.03, 0.015, 0.015, 0.015) - 11. Control effort
    % tracking_cost = tracking_cost + u1'*w_u*u1 + u2'*w_u*u2;
    
    % Phase-dependent cost combination
    J = J + (1 - phase_param) * stabilization_cost + phase_param * tracking_cost;
end
...
% STABILIZATION TERMINAL COST - ARRANGED BY DESCENDING WEIGHT
stab_terminal_cost = 0;

% Weight: 100.0 - Payload velocity final
payload_vel_final = sqrt(vNpT^2 + vEpT^2 + vDpT^2);
stab_terminal_cost = stab_terminal_cost + 100.0 * payload_vel_final^2;

% Weight: 35.0 - UAV velocities
stab_terminal_cost = stab_terminal_cost + 35.0 * (vN1T^2 + vE1T^2 + vD1T^2 + vN2T^2 + vE2T^2 + vD2T^2);

% Weight: 30.0 - Payload position centering
payload_center_NT = (N1T + N2T) / 2;
payload_center_ET = (E1T + E2T) / 2;
stab_terminal_cost = stab_terminal_cost + 30.0 * ((NpT - payload_center_NT)^2 + (EpT - payload_center_ET)^2);

% TRACKING TERMINAL COST - ARRANGED BY DESCENDING WEIGHT
track_terminal_cost = 0;

% Weight: 200.0 - Terminal altitude coordination
altitude_difference_T = D1T - D2T;
track_terminal_cost = track_terminal_cost + 200.0 * altitude_difference_T^2;

% Weight: 150.0 - Terminal position accuracy
...
% Weight: 80.0 - Terminal formation separation
...
% Weight: 50.0 - Terminal velocity coordination
track_terminal_cost = track_terminal_cost + 50.0 * ((vN1T - vN2T)^2 + (vE1T - vE2T)^2 + (vD1T - vD2T)^2);

% Weight: 40.0 - Terminal individual velocity minimization
...
% Weight: 25.0 - Terminal payload stability
track_terminal_cost = track_terminal_cost + 25.0 * (vNpT^2 + vEpT^2 + vDpT^2);

% Weight: 20.0 - Terminal yaw coordination
...
% Weight: 15.0 - Terminal attitude coordination
...
% Phase-dependent terminal cost
J = J + (1 - phase_param) * stab_terminal_cost + phase_param * track_terminal_cost;
...
tol_position = 0.15; % Tighter position tolerance for tracking phase (15 cm)
k = 1;
stabilized = false;
tracking_phase = false;

fprintf('Starting two-phase CasADi MPC...\n');
fprintf('Phase 1: Stabilization - Minimize payload oscillations (up to 100 steps)\n');
fprintf('Phase 2: Position tracking - Move to target while maintaining coordination (up to 350 steps)\n');
...
while k <= MAX_STEPS
    ...
    if ~stabilized && (payload_vel < tol_vel && uav1_vel < tol_uav_vel && uav2_vel < tol_uav_vel)
        stabilized = true;
        tracking_phase = true;
        fprintf('\n*** SWITCHING TO TRACKING PHASE at step %d! ***\n', k);
        fprintf('Stabilization complete. Now moving to target position.\n');
    elseif ~stabilized && k >= 100
        % Force switch to tracking after 100 steps even if not fully stabilized
        stabilized = true;
        tracking_phase = true;
        fprintf('\n*** FORCING SWITCH TO TRACKING PHASE at step %d! ***\n', k);
        fprintf('Maximum stabilization steps reached. Moving to tracking phase.\n');
    end
    ...
    if tracking_phase
        ...
        % Set phase parameter to tracking (1)
        opti.set_value(phase_param, 1);
    else
        fprintf('Step %d/%d [STABILIZING] | Payload vel: %.3f m/s | UAV1 vel: %.3f m/s | UAV2 vel: %.3f m/s\n', ...
            k, MAX_STEPS, payload_vel, uav1_vel, uav2_vel);
        
        % Set phase parameter to stabilization (0)
        opti.set_value(phase_param, 0);
    end
    ...
    % Check convergence for tracking phase
    if tracking_phase && position_error < tol_position && payload_vel < tol_vel
        ...
        break;
    elseif ~tracking_phase && k > 1 && payload_vel < tol_vel && uav1_vel < tol_uav_vel && uav2_vel < tol_uav_vel
        % Continue to next iteration if only stabilized but not tracking yet
    end
...
for i = 1:6
    face_vertices = vertices(faces(i,:),:) + [Np_traj(1), Ep_traj(1), Dp_traj(1)];
    payload_box_faces(i) = patch('XData', face_vertices(:,1), 'YData', face_vertices(:,2), 'ZData', face_vertices(:,3), ...
