...
% Rollout over horizon given current x0 and control sequence 
rollout = @(x0,U) simulate_rollout_2uav(x0,U,N,nu,f_step);

%% ========= Cost function for stabilization =========
function [J,Xseq] = stage_cost_stabilization(x0,U,N,nu,rollout,w_u)
    Xseq = rollout(x0,U);
    J = 0;
    ...
    for k=1:N
        ...
        % PRIMARY GOAL: Kill payload oscillations
        J = J + 50.0 * (vNp^2 + vEp^2 + vDp^2);  
        
        payload_center_N = (N1 + N2) / 2;  
        payload_center_E = (E1 + E2) / 2;  

        %  only this part is changed from 15 to 150
...
function [J,Xseq] = stage_cost_tracking(x0,U,N,nu,rollout,w_u,Ndes,Edes,Ddes)
    Xseq = rollout(x0,U);
    J = 0;
    ...
    for k=1:N
        ...
        % Tracking oda cost funtion
        
        % 1. PRIMARY: Formation center tracking to target (MODERATE PRIORITY)
        uav_center_N = (N1 + N2) / 2;
        uav_center_E = (E1 + E2) / 2;
        uav_center_D = (D1 + D2) / 2;
        
        % Smooth position tracking (reduced weight for gentle movement)
        dist_to_target = sqrt((uav_center_N - Ndes)^2 + (uav_center_E - Edes)^2 + (uav_center_D - Ddes)^2);
        position_weight = 12.0 * (1.0 + 1.5*exp(-1.5*dist_to_target)); % Smoother approach
        J = J + position_weight * ((uav_center_N - Ndes)^2 + (uav_center_E - Edes)^2 + (uav_center_D - Ddes)^2);
        
        % 2.  UAV horizontal separation 
        horizontal_separation = sqrt((N1 - N2)^2 + (E1 - E2)^2);
        separation_error = horizontal_separation - dis_between_uav;
        J = J + 50.0 * separation_error^2;  % Very strong penalty 
        
        % 3. Maintain exact same altitude 
        altitude_difference = D1 - D2;
        J = J + 120.0 * altitude_difference^2;  % 60 to 120
        
        % 4. 
        vel1_magnitude = sqrt(vN1^2 + vE1^2 + vD1^2);
        vel2_magnitude = sqrt(vN2^2 + vE2^2 + vD2^2);
        J = J + 25.0 * max(0, vel1_magnitude - max_individual_vel)^2;  %  one and only if vel1 > max
        J = J + 25.0 * max(0, vel2_magnitude - max_individual_vel)^2;  %  if vel2 > max
        J = J + 8.0 * (vel1_magnitude^2 + vel2_magnitude^2);  
        
        % 5. CRITICAL: Perfect velocity coordination between UAVs (HIGH PRIORITY)
        J = J + 30.0 * ((vN1 - vN2)^2 + (vE1 - vE2)^2 + (vD1 - vD2)^2);
        
        % 6. Smooth formation 
        max_formation_vel = 0.3;  % Maximum formation velocity toward target
        error_N = Ndes - uav_center_N;
        error_E = Edes - uav_center_E;
        error_D = Ddes - uav_center_D;
        
        % edhu dha padikanum ena nu 
        ...
        J = J + 10.0 * ((avg_vel_N - desired_vel_N)^2 + (avg_vel_E - desired_vel_E)^2 + (avg_vel_D - desired_vel_D)^2);
        
        % 7. ROMBA ROMBA MUKYAM Payload damping 
        J = J + 80.0 * (vNp^2 + vEp^2 + vDp^2);  % increase paniruka 
        ...
        if k > 1
            ...
            J = J + 40.0 * (payload_accel_N^2 + payload_accel_E^2 + payload_accel_D^2);
        end
        
        % 8. Keep payload  centered 
        payload_center_N = (N1 + N2) / 2;
        payload_center_E = (E1 + E2) / 2;
        J = J + 15.0 * ((Np - payload_center_N)^2 + (Ep - payload_center_E)^2);

        % 9. Attitude stability prevent aggressive maneuvers veanum na 0 panu bayangara aggressive ah erupanga 
        J = J + 5.0 * (phi1^2 + theta1^2 + phi2^2 + theta2^2);
        
        % 10. Angular velocity smooth ah thirumbunga 
        J = J + 3.0 * (p1^2 + q1^2 + r1^2 + p2^2 + q2^2 + r2^2);
        
        % 11. Control effort 
        J = J + u1'*w_u*u1 + u2'*w_u*u2;
        
        % 12. yaw use panlana summa suthitu eruku 
        % yaw_difference = psi1 - psi2;
        % % Normalize yaw difference to [-pi, pi]
        % yaw_difference = atan2(sin(yaw_difference), cos(yaw_difference));
        % J = J + 8.0 * yaw_difference^2;
    end
    
    %  TERMINAL COST romba romba mukyam
    xT = Xseq(:,N+1);
    N1T = xT(1); N2T = xT(13); E1T = xT(3); E2T = xT(15); D1T = xT(5); D2T = xT(17);
    vNpT = xT(26); vEpT = xT(28); vDpT = xT(30);
    vN1T = xT(2); vE1T = xT(4); vD1T = xT(6);
    vN2T = xT(14); vE2T = xT(16); vD2T = xT(18);
    ...
    % Terminal position acc maintain panradhuku
    uav_center_NT = (N1T + N2T) / 2;
    uav_center_ET = (E1T + E2T) / 2;
    uav_center_DT = (D1T + D2T) / 2;
    J = J + 150 * ((uav_center_NT - Ndes)^2 + (uav_center_ET - Edes)^2 + (uav_center_DT - Ddes)^2);
    
    % olunga rendu axis laium distance maintain panunga 
    horizontal_separation_T = sqrt((N1T - N2T)^2 + (E1T - E2T)^2);
    separation_error_T = horizontal_separation_T - dis_between_uav;
    J = J + 80.0 * separation_error_T^2;
    
    % orey height maintain panunga increase paniruku weights ah 
    altitude_difference_T = D1T - D2T;
    J = J + 200.0 * altitude_difference_T^2;  % same hight la erunga 
    
    % rendu uav um same vel use panunga
    J = J + 50.0 * ((vN1T - vN2T)^2 + (vE1T - vE2T)^2 + (vD1T - vD2T)^2);
    
    % payload velocity minimize panu version 2 
    vel1_magnitude_T = sqrt(vN1T^2 + vE1T^2 + vD1T^2);
    vel2_magnitude_T = sqrt(vN2T^2 + vE2T^2 + vD2T^2);
    J = J + 40.0 * (vel1_magnitude_T^2 + vel2_magnitude_T^2);
    
    % payload oda velocity stop panu
    J = J + 25.0 * (vNpT^2 + vEpT^2 + vDpT^2);
    
    % Terminal attitude coordination (medium priority)
    J = J + 15.0 * (phi1T^2 + theta1T^2 + phi2T^2 + theta2T^2);
    
    % edhu theva ila use panla na summa uav suthitu erukum
    % yaw_difference_T = psi1T - psi2T;
    % yaw_difference_T = atan2(sin(yaw_difference_T), cos(yaw_difference_T));
    % J = J + 20.0 * yaw_difference_T^2;
...
function xnext = step_dynamics_2uav(x,u,Ixx,Iyy,Izz,dt,m,mP,L,k,b,g)
    ...
    % %% FIX 1: Use rigid-body dynamics with gyro coupling for p_dot,q_dot,r_dot
    ...
    % %% FIX 2: Convert (p,q,r) -> (phi_dot,theta_dot,psi_dot) via W^{-1} (ZYX) and integrate Euler angles
    % UAV1
...
%% MPC loop ---------------------------------------------------------------
tol_vel = 0.05;   % Stop panu
tol_uav_vel = 0.1; % Stop rendu uav vel um kamiya erukum pothu
tol_position = 0.15; % edhuku kela erundha stop panidu
k = 1;
stabilized = false;
tracking_phase = false;

fprintf('Starting two-phase MPC...\n');
fprintf('Phase 1: Stabilization - Minimize payload oscillations (up to 100 steps)\n');
fprintf('Phase 2: Position tracking - Move to target while maintaining coordination (up to 200 steps)\n');
...
while k <= MAX_STEPS
    ...
    if tracking_phase
        ...
        % Use tracking cost function
        obj = @(U) stage_cost_tracking(x,U,N,nu,rollout,w_u,Ndes,Edes,Ddes);
    else
        fprintf('Step %d/%d [STABILIZING] | Payload vel: %.3f m/s | UAV1 vel: %.3f m/s | UAV2 vel: %.3f m/s\n', ...
            k, MAX_STEPS, payload_vel, uav1_vel, uav2_vel);
        
        % Use stabilization cost function
        obj = @(U) stage_cost_stabilization(x,U,N,nu,rollout,w_u);
    end
...
title('Two-Phase UAV-Payload MPC (NED)');
...
for i = 1:length(t)
    ...
    title(['Two-Phase UAV-Payload MPC (NED) - ', time_text]);
