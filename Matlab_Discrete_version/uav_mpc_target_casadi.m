...
start_Np    = (pN1 + pN2) / 2;   
start_vNp   = 0.0;               
start_Ep    = 1.3;    % try the swing front or back 0.8 or -0.8
start_vEp   = 0.1;               
start_Dp    = pD1 + L - 0.3;          
start_vDp   = 0.0;               
...
for k = 1:N
    ...
    % STABILIZATION COST (when phase_param = 0)
    stabilization_cost = 0;
    
    % PRIMARY GOAL: Kill payload oscillations VERY QUICKLY (highest priority)
    stabilization_cost = stabilization_cost + 50.0 * (vNp^2 + vEp^2 + vDp^2);
    
    % Add payload position penalty to center it between UAVs
    payload_center_N = (N1 + N2) / 2;
    payload_center_E = (E1 + E2) / 2;
    stabilization_cost = stabilization_cost + 150.0 * ((Np - payload_center_N)^2 + (Ep - payload_center_E)^2);
    ...
    % TRACKING COST (when phase_param = 1)
    tracking_cost = 0;
