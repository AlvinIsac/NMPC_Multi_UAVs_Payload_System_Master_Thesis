...
function [J,Xseq] = stage_cost_2uav(x0,U,N,nu,rollout,w_u)
    ...
    for k=1:N
        ...
        payload_center_N = (N1 + N2) / 2;  % Desired payload North position
        payload_center_E = (E1 + E2) / 2;  % Desired payload East position
...
function xnext = step_dynamics_2uav(x,u,Ixx,Iyy,Izz,dt,m,mP,L,k,b,g)
    ...
    if d1 > 1e-6 % 0.000001 or try with 0 
        u1t = r1p / d1;
        % Spring force
    ...
    % Compute accelerations for UAV1 (use th1 / th2 variable names)
    ...
    % Angular accel for UAV1
    p1_dot = tau_phi1 / Ixx;
    q1_dot = tau_theta1 / Iyy;
    r1_dot = tau_psi1 / Izz;

    % Compute accelerations for UAV2
    ...
    % Angular accel for UAV2
    p2_dot = tau_phi2 / Ixx;
    q2_dot = tau_theta2 / Iyy;
    r2_dot = tau_psi2 / Izz;
    ...
    p1 = p1 + p1_dot*dt; phi1 = phi1 + p1*dt;
    q1 = q1 + q1_dot*dt; th1 = th1 + q1*dt;
    r1 = r1 + r1_dot*dt; psi1 = psi1 + r1*dt;
    ...
    p2 = p2 + p2_dot*dt; phi2 = phi2 + p2*dt;
    q2 = q2 + q2_dot*dt; th2 = th2 + q2*dt;
    r2 = r2 + r2_dot*dt; psi2 = psi2 + r2*dt;

    % Payload dynamics: forces from both UAV tethers + gravity
    aNp = - (F1(1) + F2(1)) / mP;
    aEp = - (F1(2) + F2(2)) / mP;
    aDp = - (F1(3) + F2(3)) / mP + g;
...
while k <= MAX_STEPS
    ...
    if payload_vel < tol_vel && uav1_vel < tol_uav_vel && uav2_vel < tol_uav_vel
