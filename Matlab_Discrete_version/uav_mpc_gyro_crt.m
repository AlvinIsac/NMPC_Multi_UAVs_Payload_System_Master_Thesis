...
function xnext = step_dynamics_2uav(x,u,Ixx,Iyy,Izz,dt,m,mP,L,k,b,g)
    ...
    if d1 > 1e-6
        ...
        F1_damping = b * dot(vel_rel1, u1t) * u1t; % only damping along the tether direction
    ...
    if d2 > 1e-6
        ...
        F2_damping = b * dot(vel_rel2, u2t) * u2t; % only damping along the tether direction    
...
subplot(3,1,1); 
plot(t,rad2deg(phi1_traj),'b','LineWidth',1.6); hold on; 
plot(t,rad2deg(phi2_traj),'g','LineWidth',1.2); 
xlim([t(1), t(end)]);
ylabel('\phi [deg]'); grid on; legend('UAV1','UAV2');

subplot(3,1,2); 
plot(t,rad2deg(theta1_traj),'b','LineWidth',1.6); hold on; 
plot(t,rad2deg(theta2_traj),'g','LineWidth',1.2); 
xlim([t(1), t(end)]);
ylabel('\theta [deg]'); grid on;

subplot(3,1,3); 
plot(t,rad2deg(psi1_traj),'b','LineWidth',1.6); hold on; 
plot(t,rad2deg(psi2_traj),'g','LineWidth',1.2); 
xlim([t(1), t(end)]);
