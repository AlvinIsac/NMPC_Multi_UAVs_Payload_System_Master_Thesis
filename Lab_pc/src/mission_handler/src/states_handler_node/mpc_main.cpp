...
public:
  NMpcController() : Node("Nmpc_controller") {
    ...
    sub_uav3_tension_ = create_subscription<JointForceTorque>(
        ...
        std::bind(&NMpcController::uav3TensionCb, this, std::placeholders::_1));

    sub_uav2_tension_ = create_subscription<JointForceTorque>(
        "/x500_2/rod_force_torque", 10,
        std::bind(&NMpcController::uav2TensionCb, this, std::placeholders::_1));

    // Publishers
    pub_setpoint_3 = create_publisher<InputSetpoint>("/x500_3/input_setpoint", 10);
    pub_setpoint_2 = create_publisher<InputSetpoint>("/x500_2/input_setpoint", 10);
...
private:
  ...
  void declare_parameters() {
    ...
    declare_parameter("N",  25);         // horizon
...
declare_parameter("w_du_tau_yaw",    0.0);
  ...
  void controlLoop() {
    ...
    try {
      ...
      // Update previous control for next ?u cost
      ...
      // Debug output
      // std::cout << "[MPC UAV2] T = " << thrust_uav2 << " | ?? = " << tau_roll_uav2 
      //           << " | ?? = " << tau_pitch_uav2 << " | ?? = " << tau_yaw_uav2 << std::endl;
      // std::cout << "[MPC UAV3] T = " << thrust_uav3 << " | ?? = " << tau_roll_uav3
      //           << " | ?? = " << tau_pitch_uav3 << " | ?? = " << tau_yaw_uav3 << std::endl;
  ...
  // --------------------------------------------------------------------------
  // Build MPC solver (NED dynamics with yaw coupling + ?u cost)
